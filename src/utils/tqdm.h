...
class tqdm {
    private:
        ...
        chrono::time_point<chrono::system_clock> t_first = chrono::system_clock::now();
        chrono::time_point<chrono::system_clock> t_old = chrono::system_clock::now();
        int n_old = 0;
        vector<double> deq_t;
        vector<int> deq_n;
        int nupdates = 0;
    ...
    public:
        ...
        void reset() {
            t_first = chrono::system_clock::now();
            t_old = chrono::system_clock::now();
            n_old = 0;
            deq_t.clear();
            deq_n.clear();
            period = 1;
        ...
        void set_label(string label_) { label = label_; }
        void disable_colors() {
            color_transition = false;
            use_colors = false;
        }
        ...
        void progress(int curr, int tot) {
            if(is_tty && (curr%period == 0)) {
                ...
                double dt = ((chrono::duration<double>)(now - t_old)).count();
                double dt_tot = ((chrono::duration<double>)(now - t_first)).count();
                int dn = curr - n_old;
                n_old = curr;
                t_old = now;
                ...
                if (use_ema) {
                    ...
                    double dtsum = accumulate(deq_t.begin(),deq_t.end(),0.);
                    int dnsum = accumulate(deq_n.begin(),deq_n.end(),0.);
                    avgrate = dnsum/dtsum;
                }
                ...
                if (nupdates > 10) {
                    period = (int)( min(max((1.0/25)*curr/dt_tot,1.0), 5e5));
                    smoothing = 25*3;
                }
                ...
                for (int i = 0; i < ifills; i++) cout << bars[8];
                if (!in_screen and (curr != tot)) printf("%s",bars[(int)(8.0*(fills-ifills))]);
                for (int i = 0; i < width-ifills-1; i++) cout << bars[0];
