...
using namespace cec17_internal;
...
void cec17_test_func(double *x, double *f, int nx, int mx,int func_num)
{
	...
	if (ini_flag==0)
	{
		...
		/* Load Matrix M*/
		sprintf(FileName, "%s/M_%d_D%d.txt", CEC24_DATA_DIR, func_num,nx);
		...
		/* Load shift_data */
		sprintf(FileName, "%s/shift_data_%d.txt", CEC24_DATA_DIR, func_num);
		fpt = fopen(FileName,"r");
		if (fpt==NULL)
		{
			printf("\n Error: Cannot open input file for reading \n");
		...
		{
			sprintf(FileName, "%s/shuffle_data_%d_D%d.txt", CEC24_DATA_DIR, func_num, nx);
			fpt = fopen(FileName,"r");
			if (fpt==NULL)
			{
				printf("\n Error: Cannot open input file for reading \n");
