...
static double *OShift = NULL, *M = NULL, *y = NULL, *z = NULL, *x_bound = NULL;
static int ini_flag = 0, n_flag = 0, func_flag = 0, *SS = NULL;

static int read_double(FILE *file, double *out) {
	return fscanf(file, " %lf", out);
}

static int read_int(FILE *file, int *out) {
...
using namespace cec22_internal;
...
void cec22_test_func(double *x, double *f, int nx, int mx,int func_num)
{
	...
	if (ini_flag==0)
	{
		...
		/* Load Matrix M*/
		sprintf(FileName, "%s/M_%d_D%d.txt", CEC22_DATA_DIR, func_num, nx);
		...
		if (func_num<9)
		{
			...
			for (i=0; i<nx*nx; i++)
			{
				if (read_double(fpt, &M[i]) != 1)
				{
					fprintf(stderr, "\nError: Failed to read M_%d_D%d.txt (index %d).\n", func_num, nx, i);
					read_failed = 1;
					break;
				}
			}
		...
		/* Load shift_data */
		sprintf(FileName, "%s/shift_data_%d.txt", CEC22_DATA_DIR, func_num);
		fpt = fopen(FileName,"r");
		if (fpt==NULL)
		{
		...
		if (func_num<9)
		{
			...
			for(i=0;i<nx;i++)
			{
				if (read_double(fpt, &OShift[i]) != 1)
				{
					fprintf(stderr, "\nError: Failed to read shift_data_%d.txt (index %d).\n", func_num, i);
					read_failed = 1;
					break;
				}
			}
		...
		else
		{
			...
			for(i=0;i<cf_num-1;i++)
			{
				for (j=0;j<nx;j++)
				{
					if (read_double(fpt, &OShift[i*nx+j]) != 1)
					{
						fprintf(stderr, "\nError: Failed to read shift_data_%d.txt (index %d).\n", func_num, i*nx + j);
						read_failed = 1;
						break;
			...
			for (j=0;j<nx;j++)
			{
	 			if (read_double(fpt, &OShift[nx*(cf_num-1)+j]) != 1)
	 			{
	 				fprintf(stderr, "\nError: Failed to read shift_data_%d.txt (index %d).\n", func_num, (cf_num-1)*nx + j);
	 				fclose(fpt);
	 				return;
	 			}
		...
		{
			sprintf(FileName, "%s/shuffle_data_%d_D%d.txt", CEC22_DATA_DIR, func_num, nx);
			fpt = fopen(FileName,"r");
			if (fpt==NULL)
			{
			...
			for(i=0;i<nx;i++)
			{
				if (read_int(fpt, &SS[i]) != 1)
				{
					fprintf(stderr, "\nError: Failed to read shuffle_data_%d_D%d.txt (index %d).\n", func_num, nx, i);
					read_failed = 1;
					break;
				}
			}
