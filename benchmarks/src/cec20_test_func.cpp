...
using namespace cec20_internal;
...
//     printf("\n Input func_num %d Running fun num%d  \n", func_num0, func_num);
	...
	if (ini_flag==0)
	{
		...
		/* Load Matrix M*/
		sprintf(FileName, "%s/M_%d_D%d.txt", CEC20_DATA_DIR, func_num,nx);
		...
		/* Load shift_data */
		sprintf(FileName, "%s/shift_data_%d.txt", CEC20_DATA_DIR, func_num);
		fpt = fopen(FileName,"r");
		if (fpt==NULL)
		{
		...
		{
			sprintf(FileName, "%s/shuffle_data_%d_D%d.txt", CEC20_DATA_DIR, func_num, nx);
			fpt = fopen(FileName,"r");
			if (fpt==NULL)
			{
